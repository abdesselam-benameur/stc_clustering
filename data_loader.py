...
def load_stackoverflow(data_path="data/stackoverflow/", use_SIF=True):
    ...
    # load SO embedding
    with open(data_path + "vocab_withIdx.dic", "r", encoding="utf-8") as inp_indx, open(
        data_path + "vocab_emb_Word2vec_48_index.dic", "r", encoding="utf-8"
    ) as inp_dic, open(
        data_path + "vocab_emb_Word2vec_48.vec", encoding="utf-8"
    ) as inp_vec:
...
def load_search_snippet2(data_path="data/SearchSnippets/", use_SIF=True):
    ...
    # load SO embedding
    with open(
        data_path + "SearchSnippets_vocab2idx.dic", "r", encoding="utf-8"
    ) as inp_indx:
        pair_dic = inp_indx.readlines()
        word_index = {}
        for pair in pair_dic:
...
def load_biomedical(data_path="data/Biomedical/", use_SIF=True):
    ...
    # load SO embedding
    with open(
        data_path + "Biomedical_vocab2idx.dic", "r", encoding="utf-8"
    ) as inp_indx:
