...
def autoencoder(dims, act=tf.nn.leaky_relu, init="glorot_uniform"):
    ...
    return tf.keras.models.Model(inputs=x, outputs=y, name="AE"), tf.keras.models.Model(
        inputs=x, outputs=h, name="encoder"
    )
            ...
            + (
                tf.keras.backend.sum(
                    tf.keras.backend.square(
                        tf.keras.backend.expand_dims(inputs, axis=1) - self.clusters
                    ),
                    axis=2,
                )
                / self.alpha
...
class STC(object):
    def __init__(self, dims, n_clusters=20, alpha=1.0, init="glorot_uniform"):
            ...
            class PrintACC(tf.keras.callbacks.Callback):
                ...
                def on_epoch_end(self, epoch, logs=None):
                    ...
                    feature_model = tf.keras.models.Model(
                        self.model.input, self.model.get_layer("encoder_3").output
                    )
        ...
        for ite in range(int(maxiter)):
            if ite % update_interval == 0:
                ...
                if y is not None:
                    ...
                    print(
                        "Iter %d: acc = %.5f, nmi = %.5f" % (ite, acc, nmi),
                        " ; loss=",
                        loss,
                    )
...
if __name__ == "__main__":
    ...
    for i in range(0, 5):
        ...
        parser.add_argument(
            "--ae_weights", default="data/SearchSnippets/results/ae_weights12.h5"
        )
        parser.add_argument("--save_dir", default="data/SearchSnippets/results/")
        ...
        # load dataset
        ####################################################################################
        x, y = load_data(args.dataset,use_SIF=True)
        n_clusters = len(np.unique(y))

        print("les shapes avant : ", x.shape, y.shape)
        # print(x)
        # ratios =  [random.random() for _ in range(n_clusters)]
        # ratios = [0.40,0.14,0.777,0.65,0.80,0.40,0.50,0.28]
        # list of ratios for each class that reduce the dataset to 50% of the original size
        # ratios = [0.5 for _ in range(n_clusters)]

        # x, y = reduce_data(x, y, ratios, n_clusters)
        # undersample = RandomUnderSampler(sampling_strategy="auto")
        # x, y = undersample.fit_resample(x, y)
        # print("les shapes apres : ",x.shape, y.shape)
        ...
        nmi_list.append(nmi)
        acc_list.append(acc)
        average_nmis = sum(nmi_list) / len(nmi_list)
        average_accs = sum(acc_list) / len(acc_list)
        # get the std of the nmi and acc
        std_nmi = np.std(nmi_list)
        std_acc = np.std(acc_list)
        # print the average nmi and acc and the std
        print("average nmi:", average_nmis)
        print("average acc:", average_accs)
